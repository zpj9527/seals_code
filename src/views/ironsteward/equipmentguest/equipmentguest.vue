<template>
    ...
    <div class=" equipmentguest">
                ...
                <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
                        ...
                        <el-row>
                            ...
                            <el-col :span="6" class="eq_Project">
                                <div class="hotel_num hotel_num_3">
                                    <p>房型：</p>
                                    <el-select v-model="form.region" multiple  placeholder="请选择内容" >
                            ...
                            <el-col :span="17">
                                ...
                                <el-table
                                    ...
                                    <el-table-column prop="room_type_name"  label="房型" width="180" sortable>
                                    </el-table-column>
                                    <el-table-column prop="room_number" label="房间" width="180" sortable>
                                    </el-table-column>
                                    <el-table-column prop="control_scheme" label="设备方案" sortable>
                                    </el-table-column>
                        ...
                        <el-row>
                            ...
                            <el-col :span="24">
                                ...
                                <el-table
                                    ...
                                    <el-table-column prop="control_scheme" label="方案编号/名称" >
                                    </el-table-column>
                                    <el-table-column prop="control_scheme" label="ID" >
                                    </el-table-column>

                                    <el-table-column prop="equipment_type" label="设备">
                                    </el-table-column>
                                    <el-table-column prop="manufacture" label="厂家">
                                    </el-table-column>
                                    <el-table-column prop="device_id" label="内容">
                                    </el-table-column>
                                    <el-table-column prop="created" label="创建时间"> 
                                    </el-table-column>
                                    <el-table-column prop="last_update" label="更新时间">
                                    </el-table-column>
                    ...
                    <el-tab-pane label="独立设备设置" name="second">
                        <el-row>
                            <el-col :span="24" >
                                <div>
                                    <div class="nav_select inline-block" style="display: none;">
                                        <p>设备方案：</p>
                                    ...
                                    <div class="nav_select inline-block">
                                        ...
                                        <el-select v-model="form_room_set.room_number" placeholder="请选择" @change="get_panel(form_room_set.room_number)">
                                            <el-option v-for="item in roomList" :key="item.id"
                                                :label="item.room_number"
                                ...
                                <div>
                                    <el-tabs v-model="activeName2" @tab-click="handleClick">
                                        <el-tab-pane v-for="(item,index) in panelList.not_common_list" :key="index" :label=" item.equipment_name " :name="item.equipment_name">
                                             <div class="nav_pane">
                                                <div class="hotel_num pane_cell">
                                                    <div class="url inline-block" style="font-size: 14px;"> &nbsp;厂 家 ：</div>
                                                   
                                                    <el-select v-model=" item.parameter.manufacture " placeholder="请选择厂家" style="width: 20%;">
                                                ...
                                                <div class="hotel_num pane_cell" >
                                                    ...
                                                    <el-input
                                                        placeholder="请输入内容"
                                                        v-model=" item.parameter.manufacture_device_id "
                                                        clearable style="width: 20%;">
                                                    </el-input>
                                                </div>
                                                <div class="btn_group " style="overflow: hidden;">
                                                    <el-button class="pull-right" size="small" @click="confirm_device(item.router, { 'room_num': form_room_set.room_number,'manufacture':item.parameter.manufacture , 'manufacture_device_id': item.parameter.manufacture_device_id })"  style="background-color: #4488E9;" type="primary" >提交</el-button>
                                                </div>
                                            </div>
                                        </el-tab-pane> 
                                    </el-tabs>
                                ...
                                <div>
                                    <el-tabs v-model="activeName3"  @tab-click="handleClick">
                                        <el-tab-pane v-for="(item,index) in panelList.is_common_list" :key="index" :label=" item.equipment_name " :name="item.equipment_name" >
                                            <div class="nav_pane">
                                                <div class="hotel_num pane_cell">
                                                    <div class="url inline-block" style="font-size: 14px;"> &nbsp;厂 家 ：</div>
                                                    <el-select v-model=" item.parameter.manufacture " placeholder="请选择厂家" style="width: 20%;">
        ...
        <!-- 筛选选中的房间的弹窗 :before-close="handleClose"-->
        <el-dialog title="房间" :visible.sync="checkRoomVisible" width="30%" >
            <div>
                <p v-for="(item,index) in form.region" :key="index">{{ item }}</p>
                <ul  class="roomCheckList">
                    <li v-for="(item,index) in form.region" :key="index" class="roomItem"></li>
                </ul>
            </div>

            <span slot="footer" class="dialog-footer">
                <el-button @click="checkRoomVisible = false">取 消</el-button>
                <el-button type="primary" @click="checkRoomVisible = false">确 定</el-button>
            </span>
        </el-dialog>
...
<script>
    export default {
        ...
        created(){
            this.get_hotel(JSON.parse(localStorage.getItem('userInfo')).hotel_id);
            // 房型列表
        ...
        methods:{
            // 获取单个房间必填信息和其它设备
            get_panel(room_num){
                let that = this , url = that.UrLHeader + 'equipment/ht/room/query_room_control_scheme_eqt/';
                that.$axios.post(url , { "hotel_id": JSON.parse(localStorage.getItem('userInfo')).hotel_id, "room_number":room_num }).then(function(res) {
                        // 判断返回数据是否有数据
                        if(res.data.message =='success'){  
                            that.panelList = res.data.data;
                            // panel重命名
                            that.activeName2 = res.data.data.not_common_list[0].equipment_name;
                            // 其它设备信息
                            that.activeName3 = res.data.data.is_common_list[0].equipment_name;
                        }else{
                            that.$message.error('解除方案失败');
                        }
                    }).catch((error) => { });
            },
            /* 确认提交单个设备信息 */
            confirm_device(router,params){
                let that = this;
                console.log(params);
                that.$axios.post(router,params).then(function(res) {
                ...
                _this.$axios.get(getUrl).then(function(res) {
                    if(res.data.message =='success'){
                        ...
                        if(res.data.data.results){ 
                            // 遍历取值
                            let getAll = function(arr,id) {
                                arr.filter(item => {  
                                    if(item.id && item.id === id){ 
                                         _this.eq_name = item.equipment_name ;
                                         
                                    }
                                    if(item.hierarchy && item.hierarchy.length) {
                                        getAll(item.hierarchy,id);
                                    }
                                })
                            };
                            for(let i in res.data.data.results){
                                res.data.data.results[i].manufacture = _this.const_base[res.data.data.results[i].manufacture];
                                // 设备厂家
                                getAll(_this.eq_tree_list,res.data.data.results[i].equipment_type);
                                res.data.data.results[i].equipment_type = _this.eq_name;
            ...
            get_man_const(){
                ...
                _this.$axios.get(getUrl).then(function(res) {
                    if(res.data.message =='success'){
                        ...
                        // 声明厂家键值对
                        var const_base = {};
                        for (let j in _this.const_list ) {
                            let id = _this.const_list [j].const;
                            const_base[id] = _this.const_list [j].manufacture_name
                        }

                        _this.const_base = const_base;
                        console.log(_this.const_base);
