    ...
    <div>
        <el-dialog width="70%" class="preview-enter-dialog" title="预定转入住"  :visible.sync="previewToEnterVisible"
          ...
          <el-row class="top-nav">
            <span>客源:<span></span></span>
            <span>联系人:<span></span></span>
            <span>支付状态:<span>未支付</span></span>
          </el-row>
          <el-row style="margin-top: 10px">
            ...
            <el-row style="margin-top: 120px">
                ...
                <el-button style="height: 50px; width: 100px;" type="success" @click="confirmPreToEnter()">确定入住</el-button>
                </div>
            </el-row>
        </el-dialog>
        <!-- 选房号 -->
        ...
        <el-dialog class="houseTypeClass deletePadding_Class" title="入预收操作" :visible.sync="payMoneyDialog" :modal="true">
          <div style="width: 100%; height: 400px">
            <ul class="enter_pre_ul">
              ...
              <!-- <li><span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;应收: </span><span style="color: #f3565d;">{{countMoney}}</span>(预收款总计: <span style="color: #f3565d;">{{countMoney}}</span>已收:  <span>0</span>预计消费总计: {{countMoney}})</li> -->
              ...
              <li><span>预收操作:</span>
                <el-select style="width: 12vw" @focus="getPayMode()" placeholder="付款方式"  v-model="payModeValue">
                  ...
                  <!-- <el-input size="small" style="width: 8vw" v-model="countMoney"></el-input>
          ...
          <div slot="footer" class="dialog-footer">
            <el-button type="danger"  @click="payMoneyDialog=false;">打印关闭</el-button>
          </div>
        </el-dialog>
    </div>
</template>
...
export default {
    data(){
        return {
          ...
          // billUrl: 'http://192.168.2.224:9005/v1/'
    ...
    methods: {
      ...
      getInfoByAccount(){
        ...
        if(this.preBillParam.reserve_base.account_opened===1){
          ...
          let url = that.accountUrl + 'finance/account/get_info/' + id
          that.$axios({
           method : 'get',
            url : url,
            params: param
      ...
      transferAccount(){
        ...
        let url = that.accountUrl + 'finance/transfer_accounts_detail/add'
        let scopeParam = {
        ...
        addSomePay(){
          ...
          let url = that.accountUrl + 'finance/pay_detail/pay_by_charges'
        ...
        getIncomingAccount(param){
          ...
          let url = that.accountUrl + 'finance/incoming_account_code/info_list'
          that.$axios({
            method : 'get',
                url : url,
            }).then(res=>{
      ...
      //   if(this.payModeValue === 0){
      //     //等于1代表开帐了
      //     if(this.preBillParam.reserve_base.rsv_status_lable !=1){
      //       this.addAccount()//添加账户
      //     }else{
      //       this.addPayDetail()
      ...
      // },
        ...
        addChargeDetail(){
          let scopeParam = {
            cashier_id: 1,//	收银点id
            account_id: this.preBillParam.reserve_base.account_id,//
            charges: [], //消费明细字典组成的数组
          }
          let that = this
          // let url= `http://47.98.113.173:9519/v1/finance/charge_detail/add_charges`
          let url = that.accountUrl + 'finance/charge_detail/add_charges'
      ...
      confirmPreToEnter(){
        ...
        let url = billUrl + 'checkin/booking_check_in/'
        let scopeParam = _.cloneDeep(that.preBillParam) //此处必须深拷贝
        scopeParam = this.handleDeleteAttribute(scopeParam)
        that.$axios.post(url,scopeParam).then(res=>{
          console.log('res.data.messag=====带答案的===============',res.data.data,'分割',res.data.data.message)
         that.returnEnterParam = res.data.data.data
         if(res.data.data.message === 'success'){
              that.$message.warning('预定转为入住成功!')
              that.previewToEnterVisible = false
            }else{
              that.$message.warning('入住失败!')
            }
        }).catch(error=>{
        })
      },
       ...
       /**先开帐 只把消费明细传进去 */
        ...
        toUpdateCarryPayInfo(){
          ...
          let url = this.billUrl + 'checkin/update_master_base/'
          let master_base = {}
          let scopeParam = {
            master_base : {
              // returnEnterParam
              ...
              account_id: this.return_accountParam.id,
              last_account_id:  this.return_accountParam.id,
              last_pay_id:  null,
            }
          }
      ...
      concatData(param){
        ...
        addAccount(){
         ...
         this.account_param.charge_details = [{
          ...
          let url = that.accountUrl + 'finance/account/add'
          // let url = that.roomUrL + 'room_status/manual_change_room_status/'
       ...
       //删除选择框=>当只剩下一个的时候不能在删除
        ...
        validatePreviewData(){
          ...
          for(var item of this.preBillParam.rt_rate){
            if(item.room_number === ''){
                this.$message.warning('请选择房号进行排房!')
                return false
            }
            else{
              return true
        ...
        getBreakfastList(){
          ...
          // let url = that.roomUrL + 'room/get_roomnumber_list/'
        ...
        getPayMode(){
          ...
          let url = that.accountUrl + 'finance/pay_mode/info_list'
          that.$axios({
            method : 'get',
            url : url,
          }).then(res=>{
        ...
        getPayReason(){
          ...
          let url = that.accountUrl + 'finance/code_pay_for/info_list'
          that.$axios({
            method : 'get',
            url : url,
          }).then(res=>{
