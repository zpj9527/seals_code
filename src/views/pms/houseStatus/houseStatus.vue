    ...
    <el-container>
          ...
          <el-tab-pane label="房态筛选" name="first">
            <div>
              ...
              <div style="margin-top: 10px;display: flex; justify-content: space-between">
                <el-input placeholder="请输入房间号" v-model="room_no_value"  style="width: 8vw"></el-input>
                <el-button  type='primary' @click="getCardList()">查找</el-button>
              </div>
              <div>
                <el-row style="margin-top: 15px">楼栋:
                  <el-select @focus="getBuilding()" @change="getCardList()" style="width: 8vw"  v-model="building_value">
                ...
                <el-row style="margin-top: 10px">楼层:
                  <el-select @focus="getFloor()" @change="getCardList()" style="width: 8vw"  v-model="floor_value">
                    <el-option label="全部" value=""></el-option>
                    <el-option
                ...
                <el-row style="margin-top: 10px">房型:
                  <el-select style="width: 8vw" @focus="getRoomType()" @change="getCardList()"  v-model="roomType_value">
                    <el-option label="全部" value=""></el-option>
                    <el-option
              ...
              <div>
                <div class="boxStyle bigDiv">
                  <div class="boxStyle"><img src="@/assets/images/pms/houseStatus/team.png"/><span class="secondLineStyle" @click="filterData('团队')" :class="{activeBlue: isActiveBlue_team}">团队</span></div>
                  <div class="boxStyle"><img src="@/assets/images/pms/houseStatus/qianFei.png"/><span class="secondLineStyle" @click="filterData('欠费')" :class="{activeBlue: isActiveBlue_qf}">欠费</span></div>
                </div>
                <div class="boxStyle bigDiv">
                  <div class="boxStyle"><img src="@/assets/images/pms/houseStatus/computer.png"/><span class="secondLineStyle" @click="filterData('电脑')" :class="{activeBlue: isActiveBlue_computer}">电脑</span></div>
                  <div class="boxStyle"><img src="@/assets/images/pms/houseStatus/timeHouse.png"/><span class="secondLineStyle" @click="filterData('钟点')" :class="{activeBlue: isActiveBlue_clock}">钟点</span></div>
                </div>
                <div class="boxStyle bigDiv">
                  ...
                  <div class="boxStyle"><img src="@/assets/images/pms/houseStatus/visitor.png"/><span class="secondLineStyle" @click="filterData('访客')" :class="{activeBlue: isActiveBlue_visitor}">访客</span></div>
                  <div class="boxStyle"><img src="@/assets/images/pms/houseStatus/selfUse.png"/><span class="secondLineStyle" @click="filterData('自用')" :class="{activeBlue: isActiveBlue_self}">自用</span></div>
                </div>
                 <div class="boxStyle bigDiv">
                  <div class="boxStyle"><img src="@/assets/images/pms/houseStatus/preLeave.png"/><span class="secondLineStyle" @click="selectData()" :class="{activeBlue: isActiveBlue_leave}">预离</span></div>
                  <div class="boxStyle"><img src="@/assets/images/pms/houseStatus/selfHelp.png"/><span style="margin-left: 5px" @click="filterData('自助机')" :class="{activeBlue: isActiveBlue_zzj}">自助机</span></div>
                </div>
                 <div class="boxStyle bigDiv">
                  <div class="boxStyle"><img src="@/assets/images/pms/houseStatus/and.png"/><span class="secondLineStyle" @click="filterData('联房')" :class="{activeBlue: isActiveBlue_lf}">联房</span></div>
                  <div class="boxStyle"><img src="@/assets/images/pms/houseStatus/preDestine.png"/><span class="secondLineStyle" @click="filterData('预定')" :class="{activeBlue: isActiveBlue_yd}">预定</span></div>
                </div>
                 <div class="boxStyle bigDiv">
                </div>
              </div>
          ...
          <el-tab-pane label="批量操作" name="second">
            ...
            <div style="height: 400px">
              ...
              <div style="margin-top: 20px">
                <el-row>
                  <el-button :disabled="isPress" @click="setBatchDirtyClean('dirty')" style="width: 84px;color: #c8c8c8" :class="{batchBtn: isShow_1}">置脏</el-button>
                  <el-button :disabled="isPress" @click="setBatchDirtyClean()" style="color: #c8c8c8" :class="{batchBtn: isShow_2}">置干净</el-button>
                </el-row>
                <el-row style="margin-top: 10px">
                  <el-button  @click="handleBatchRepairLock('repair'); repairDialog=true" :disabled="isPress" style="color: #c8c8c8" :class="{batchBtn: isShow_3}">置维修</el-button>
                  <el-button @click="handleBatchRepairLock(); lockDialog=true" :disabled="isPress" style="width: 84px;color: #c8c8c8" :class="{batchBtn: isShow_4}">锁房</el-button>
                </el-row>
              </div>
            </div>
          </el-tab-pane>
      ...
      <el-main class="right" >
        ...
        <div class="parDiv" v-if="batchOperation">
          <el-col  :span="2" class="right-card" v-for="(item, index) in cardList" :key="index">
            <!-- 净房 -->
            <div @click="cardClick(item,index)" :class="{trueMark: item.is_halt}" @mouseover="hoverIndex = index" @mouseout="hoverIndex = -1"  v-if="item.room_state === 'VC'"  class="boxActive">
              <div  style="display: flex; justify-content: space-between;">
                <span style="font-size: 16px">{{item.room_type_name}}</span>
                <!-- <span>11</span> -->
        ...
        <div class="parDiv" v-if="future_houseStatus">
          <el-col  :span="2" class="right-card" v-for="(item, index) in future_cardList" :key="index">
            ...
            <div v-if="item.room_state === 'VC'"  class="boxActive">
              <div  style="display: flex; justify-content: space-between;">
                <span style="font-size: 16px">{{item.building_name}}</span>
                <!-- <span>11</span> -->
                <span style="font-weight: bold; font-size: 16px">{{item.room_no}}</span>
              </div>
            </div>
          ...
          <el-col  :span="2" class="right-card" v-for="(item, index) in cardList" :key="index">
            <el-popover
              ...
              <div>
                ...
                <div v-if="item.room_state === 'VC'" class="pop_state"  style="width: 300px; height: 80px;">
                  <div><el-button type="primary" round size="mini">{{item.room_no}}</el-button>{{item.descript}}</div>
                  <div>
                    <el-row>
                ...
                <div v-if="item.room_state === 'OC'" style="width: 300px; height: 160px; ">
                   <div class="pop_state">
                    <div><el-button type="primary" round size="mini">{{item.room_no}}</el-button>{{item.descript}}</div>
                    <div>
                      <el-row style="display: flex; justify-content: flex-end">
                        <button class="setbtn" @click="openEnterAccount(item)">入账</button>
                      ...
                      <el-row style="margin-top: 10px; display: flex;  justify-content: flex-end">
                        <button class="setbtn" @click="openChangeRoomType(item)">换房型</button>
                        <button class="setbtn" @click="openChangeRoomNumber(item)">换房号</button>
                      </el-row>
                      <el-row style="margin-top: 10px;  display: flex;  justify-content: flex-end">
                        <button class="setbtn" @click="setOpenDoor(item); openDoorDialog=true">请求开门</button>
                  ...
                  <div  class="pop_bottom">
                    <div style="margin-top: 5px">此处放置日期</div>
                    <div>
                ...
                <!-- 浮动框的v-if====> 入住房 住脏-->
                <div v-if="item.room_state === 'OD'"   style="width: 300px; height: 160px; ">
                  <div class="pop_state">
                    <div><el-button type="primary" round size="mini">{{item.room_no}}</el-button>{{item.descript}}</div>
                    <div>
                      <el-row style="display: flex; justify-content: flex-end">
                ...
                <!-- 浮动框的v-if====> 脏房-->
                <div v-if="item.room_state === 'VD'"   style="width: 300px; height: 80px; ">
                  <div class="pop_state">
                    <div><el-button type="primary" round size="mini">{{item.room_no}}</el-button>{{item.descript}}</div>
                    <div>
                      ...
                      <el-row style="margin-top: 10px; display: flex;  justify-content: flex-end">
                        <button class="setbtn" @click="setClean(item); cleanDialog=true">置干净</button>
                        <button class="setbtn" @click="handleSetRepair(item); repairDialog=true">置维修</button>
                      </el-row>
                      <el-row style="margin-top: 10px;  display: flex;  justify-content: flex-end">
                        <button class="setbtn" @click="setOpenDoor(item); openDoorDialog=true">请求开门</button>
                ...
                <!-- 浮动框的v-if====> 维修-->
                <div v-if="item.room_state === 'OO'"   style="width: 300px; height: 80px; ">
                  <div class="pop_state">
                    <div><el-button type="primary" round size="mini">{{item.room_no}}</el-button>{{item.descript}}</div>
                    <div>
                      <el-row>
                        <button class="setbtn">结束维修</button>
                      </el-row>
                    </div>
                  </div>
                 ...
                 <!-- pop浮动框的v-if====> 锁房-->
                <div v-if="item.room_state === 'OS'"   style="width: 300px; height: 80px; ">
                  <div class="pop_state">
                    <div><el-button type="primary" round size="mini">{{item.room_no}}</el-button>{{item.descript}}</div>
                    <div>
                      <el-row>
              ...
              <div v-if="item.room_state === 'VC'"  slot="reference"  @mouseover="hoverIndex = index" @mouseout="hoverIndex = -1"   @click="popoverClick(item,index)"  :class="{popoverActive: activeClassName == index && isActive, lockCss: activeClassName == index && lockCss_active, hoverActive: index==hoverIndex}"   @dblclick="sendEnter(item);flushData()"  class="boxActive">
                <!--单个房间(小方块)第一行-->
                <div  style="display: flex; justify-content: space-between;">
                    <span style="font-size: 16px">{{item.room_type_name}}</span>
                    <!-- <span>11</span> -->
            ...
            <!-- card入住房 住净-->
             <div v-if="item.room_state === 'OC'"  @mouseover="hoverIndex = index" @mouseout="hoverIndex = -1"   @click="popoverClick(item,index)"  slot="reference" :class="{popoverActive: activeClassName == index && isActive,lockCss: activeClassName == index && lockCss_active, hoverActive: index==hoverIndex}"   @dblclick="openOrderInfo(item)"  class="boxActive_6">
                <!--单个房间(小方块)第一行-->
                <div  style="display: flex; justify-content: space-between;">
                    <span style="font-size: 16px">{{item.room_type_name}}</span>
                    <!-- <span>11</span> -->
                ...
                <div :class="{iconEnd: item.expandAttribute.isleave === 1 ? true : false}">
                  <img v-if="item.expandAttribute.isleave === 1" src="@/assets/images/pms/houseStatus/preLeave.png" style="height: 16px">
                </div>
                <div>
                </div>
              </div>
               <!-- card入住房 住脏-->
             <div v-if="item.room_state === 'OD'"  @mouseover="hoverIndex = index" @mouseout="hoverIndex = -1"   @click="popoverClick(item,index)"  slot="reference" :class="{popoverActive: activeClassName == index && isActive,lockCss: activeClassName == index && lockCss_active, hoverActive: index==hoverIndex}"   @dblclick="openOrderInfo(item)"  class="boxActive_2">
                <!--单个房间(小方块)第一行-->
                <div  style="display: flex; justify-content: space-between;">
                  <span style="font-size: 16px">{{item.room_type_name}}</span>
                  <!-- <span>11</span> -->
              ...
              <!-- card脏房 -->
             <div v-if="item.room_state === 'VD'"  @mouseover="hoverIndex = index" @mouseout="hoverIndex = -1"   @click="popoverClick(item,index)"  slot="reference" :class="{popoverActive: activeClassName == index && isActive, lockCss: activeClassName == index && lockCss_active, hoverActive: index==hoverIndex}"    class="boxActive_3">
                <!--单个房间(小方块)第一行-->
                <div  style="display: flex; justify-content: space-between;">
                    <span style="font-size: 16px">{{item.room_type_name}}</span>
                    <!-- <span>11</span> -->
              ...
              <!-- card锁定房间 -->
             <div v-if="item.room_state === 'OS'"  @mouseover="hoverIndex = index" @mouseout="hoverIndex = -1"   @click="popoverClick(item,index)" slot="reference" :class="{popoverActive: activeClassName == index && isActive,  hoverActive: index==hoverIndex}"    class="boxActive_5">
                <!--单个房间(小方块)第一行-->
                <div  style="display: flex; justify-content: space-between;">
                    <span style="font-size: 16px">{{item.room_type_name}}</span>
                    <!-- <span>11</span> -->
              ...
              <!-- 维修 -->
             <div v-if="item.room_state === 'OO'"  @mouseover="hoverIndex = index" @mouseout="hoverIndex = -1"  slot="reference" :class="{popoverActive: activeClassName == index && isActive,lockCss: activeClassName == index && lockCss_active,  hoverActive: index==hoverIndex}"   @dblclick="openOrderInfo()"  class="boxActive_4">
                <!--单个房间(小方块)第一行-->
                <div  style="display: flex; justify-content: space-between;">
                  <span style="font-size: 16px">{{item.room_type_name}}</span>
                  <!-- <span>11</span> -->
        ...
        <!--置干净dialog-->
        <el-dialog width="20%" class="houseTypeClass" title="置干净" :visible.sync="cleanDialog" :modal="false">
            <el-row>打扫人:
               <el-select  v-model="clean_value">
                <el-option
        ...
        <el-dialog width="25%"  class="houseTypeClass" title="置维修" :visible.sync="repairDialog" :modal="true">
            ...
            <el-input type="textarea" :rows="5" v-model="repairAndParam.remark"></el-input>
            <span slot="footer" class="dialog-footer">
            <el-button type="primary" v-if="btnShow" @click="setRepairAndLock();repairDialog = false">提交</el-button>
        ...
        <el-dialog width="30%" class="houseTypeClass" title="请求开门" :visible.sync="openDoorDialog" :modal="true">
          ...
          <!--新建入住单dialog-->
          <enter-dialog :show.sync= enterFormVisible :parentParam='enterBillParam' v-on:listenToPreview="getPreviewFromChild"></enter-dialog>
          <!--预定单详情dialog-->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //筛选预离的数据
      selectData(){
        this.cardList  = this.cardList.filter(item=>item.expandAttribute.isleave === 1)
        // this.cardList = array
        console.log('预离this..cardList',this.cardList)
      },
      ...
      getCardList(){
        ...
        that.$axios({
          ...
          if(res.data.message === 'success'){
            ...
            console.log('that.mapArray',that.mapArray)
      ...
      getPreInfoByRoom(param){
        ...
        let url = `http://bill.crowncrystalhotel.com/v1/booking/get_all_reserve_info/`
        // let url = `http://192.168.2.224:9005/v1/booking/all_reserve_info_dis/`
        // let url = `http://bill.crowncrystalhotel.com/v1/booking/all_reserve_info_dis/`
        that.$axios({
           method : 'get',
            url : url,
            params: scopeParams
          ...
          if(res.data.message === 'success'){
            ...
            console.log('that.preInfoParam详情',that.preInfoParam)
            //处理数据然后传给子组件
      ...
      openOrderInfo(item){
        ...
        this.getEnterInfoByRoom(item.room_no)
      },
      ...
      getEnterInfoByRoom(param){
        ...
        let that = this
        // let url = that.UrLTrueHeader + '9101' + '/v1/room_status/get_room_map_list'
        // let url = `http://192.168.2.224:9005/v1/checkin/all_master_info/`
        let url = `http://bill.crowncrystalhotel.com/v1/checkin/all_master_info/`
        that.$axios({
           method : 'get',
            url : url,
            params: scopeParams
          ...
          if(res.data.message === 'success'){
            console.log('res.data.data.results==link',res.data.data.results)
            that.enterInfoBillParam = res.data.data.results[0]
            this.linInfoParam = _.cloneDeep(this.enterInfoBillParam) //联房详情
            if(this.linInfoParam != undefined){
              this.linkHouseFornVisible = true //联房列表打开
            }else{
              this.$message.warning('获取后台数据失败!')
            }
            console.log('this.linInfoParam ===========要传入的值',this.linInfoParam)
          }else{
      ...
      getHouseFutureList(date){
        let that = this
        // let url = that.UrLHeader + 'room/get_room_state_future_list/'
        let url= `http://192.168.4.99:8002/v1/room_status/get_room_future_list/`
        that.$axios({
          method : 'get',
            url : url,
            params:{
      ...
      handleSetLock(item,index){
        ...
        this.repairAndParam.room_no_id = item.room_no_id
      ...
      //   let url= `http://price.crowncrystalhotel.com/v1/room_status/manual_change_room_status/`
      ...
      setDirty(param){
        ...
        this.repairAndParam.room_no_id = param.room_no_id
      ...
      getRepairOccupy(){
        ...
        let url = this.UrLHeader + 'room_status/get_code_room_occupy_list/'
        // let url= `http://192.168.4.99:8002/v1/room_status/get_code_room_occupy_list/`
        that.$axios({
          method : 'get',
          url : url,
        }).then(res=>{
      ...
      getRepairReason(){
        let that = this
        let parent_code = 'room_lock_reason'
        let url = this.UrLHeader_3 + 'system/settings/get_code_base_list/'
        that.$axios({
          method : 'get',
          url : url,
          params:{
      ...
      getBuilding(){
        let that = this
        let url = 'http://room.crowncrystalhotel.com/v1/' + 'room/get_room_building_list/'
      ...
      getFloor(){
        let that = this
        let url = 'http://room.crowncrystalhotel.com/v1/' + 'room/get_roomfloor_list_tree/'
        that.$axios.get(url).then(res=>{
            if(res.data.message == 'success'){
      ...
      getAllMasterInfo(){
        let that = this
        let url = `http://bill.crowncrystalhotel.com/v1/checkin/all_master_list/?page_size=999`
