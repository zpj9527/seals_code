<template>
  <!--客户-->
  <div class="warp-main">
    <el-tabs v-model="activeName2" type="border-card" class="warp-tab">
      <el-tab-pane label="客人资料" name="first">
        <div class="main-top">
          ...
          <button @click="search_customer_info">查询</button>
          <button>打印</button>
...
<script>
  export default {
    ...
    methods: {
      get_all_guest_info(){
         let that = this;
         let url = that.url + '/v1/customer/guest/get_guest_base_list/?sys_cat=G';
         that.$axios({
            method : 'get',
            url : url,
         }).then((res)=>{
