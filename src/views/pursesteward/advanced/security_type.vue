    ...
    <div class="shopping">
      ...
      <el-dialog
        ...
        <ul class="dialog_style">
          ...
          <!--<span>业务名称 ：</span>-->
          <!--<el-input v-model="param.name" placeholder="请输入业务名称" style="width: 200px;" size="mini"></el-input>-->
          <!--</li>-->
          <li>
            <i style="color: red;font-size: 18px">*</i>
            <span>中文描述 ：</span>
  ...
  export default {
    ...
    methods: {
      ...
      enter_code() {
        let that = this
        that.$axios({
          url: that.url + "/v1/system/settings/get_code_base_list/?parent_code=guarantee",
          method: "get",
        })
          .then(res => {
      ...
      //点击担保种类新增和修改后的保存
        ...
        let that = this;
        if (that.param.desc === "" || that.param.desc_en === "" || that.param.code === "") {
          ...
          that.$axios({
            ...
            .then(res => {
              if (res.data.message === "success") {
                ...
                if (that.Edite) {
                  that.hintInfo("success", "修改担保种类成功")
                } else {
                  that.hintInfo("success", "新增担保种类成功")
                }
              }
              else {
