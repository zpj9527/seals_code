      ...
      <el-dialog
        ...
        <ul class="dialog_style">
          ...
          <li>
            ...
            <el-select v-model="field_type" placeholder="请选择" style="width: 200px;" size="mini">
              <el-option
        ...
        <div class="status">
          <el-table
            ...
            <el-table-column
              ...
              label="操作">
              <template slot-scope="scope">
                <el-button size="mini" type="danger" @click="expurgate(scope.row.id,scope.$index, segment_list)"
                           disabled>删除
                </el-button>
                <el-button size="mini"
  ...
  export default {
    ...
    methods: {
      ...
      empty(srow, index, list) {
        let that = this;
        that.title = srow.model_name + "添加字段定义";
      ...
      screen() {
        let that = this
        that.$axios({
          url: that.url + "/v1/finance/bill_type/info_list",
          method: "post",
          data: {
      ...
      cancel(id, index, list) {
        ...
        that.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          list.splice(index, 1);
          that.$axios({
            url: that.url + "/v1/finance/bill_type/remove/" + id,
            method: "get",
          })
            .then(res => {
              console.log(res)
      ...
      //点击票据种类新增和修改后的保存
        ...
        if (!that.param.model_name || !that.param.code || !that.is_sys || !that.is_group || !that.is_halt) {
          ...
          let urldata = that.Edite ? (that.url + "/v1/finance/bill_type/update/" + that.id) : (that.url + "/v1/finance/bill_type/add");
          ...
          that.$axios({
            ...
            .then(res => {
              if (res.data.message == "success") {
                ...
                if (that.Edite) {
                  that.hintInfo("success", "修改票据种类成功")
                } else {
                  that.hintInfo("success", "新增票据种类成功")
                }
              }
              else {
                let reg = "^((?!object is exists).)*$";
                if (reg) {
                  that.hintInfo("warning", "单据种类已存在，请检查后重新输入")
                }
              }
            })
            .catch(error => {
      ...
      * */
      ...
      fields_list(srow, index, list) {
        let that = this
        console.log(srow, index, list);
        that.title = srow.model_name + "字段自定义"
        that.model_id = srow.id;
        that.$axios({
          url: that.url + "/v1/finance/bill_type/get_fields",
          method: "post",
          data: {
      ...
      augment() {
        ...
        if (!that.param.field_name || !that.field_type || !that.param.field_desc || !that.param.field_name_cn) {
          ...
          let urldata = that.Edite ? (that.url + "/v1/finance/bill_type/update_field/" + that.id) : (that.url + "/v1/finance/bill_type/add_field");
          console.info(urldata);
          that.$axios({
            url: urldata,
            method: "post",
