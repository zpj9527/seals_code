        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <el-button size="mini" type="danger" @click="cancel(scope.row.id,scope.$index, register_list)">删除
              </el-button>
              <el-button size="mini"
                         @click="dialogVisible = true;amend(scope.row,scope.row.id,scope.$index, register_list)">修改
              </el-button>
            </template>
          </el-table-column>
      ...
      <el-dialog
        ...
        <ul class="dialog_style">
          <li>
            ...
            <span>中文描述 ：</span>
            <el-input v-model="param.desc" placeholder="请输中文描述" style="width: 200px;" size="mini"></el-input>
          </li>
          <li>
          ...
          <li>
            <i style="color: red;font-size: 18px">*</i>
            <span>代码 ：</span>
            <el-input v-model="param.code" placeholder="请输入代码" style="width: 200px;" size="mini"></el-input>
          </li>
          <li>
            <i style="color: red;font-size: 18px">*</i>
            <span>IP地址 ：</span>
            <el-input v-model="param.mac" placeholder="例:192.168.1.2" style="width: 200px;" size="mini"
                      @blur="site"></el-input>
          </li>
          <li>
            <i style="color: red;font-size: 18px">*</i>
            <span>起始备用金 ：</span>
            <el-input v-model="param.initial_cash" placeholder="请输入起始备用金" style="width: 200px;" size="mini"
                      oninput="value=value.replace(/[^\d]/g,'')"></el-input>
          </li>
          <li>
            <i style="color: red;font-size: 18px">*</i>
            <span>所属部门 ：</span>
            <el-select v-model="belong_to_balance" placeholder="请选择所属部门" style="width: 200px;" size="mini">
              <el-option
    ...
    methods: {
      ...
      cancel(id, index, list) {
        ...
        that.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          list.splice(index, 1);
          that.$axios({
            url: that.url + "/v1/finance/cash_register/remove/" + id,
            method: "get",
          })
            .then(res => {
              console.log(res)
      ...
      //点击入账代码新增和修改后的保存
        ...
        let that = this;
        if (that.param.code === "" || that.belong_to_balance === "" || that.param.mac === "" || that.param.initial_cash === "" || that.param.desc === "") {
          that.hintInfo("warning", "*为必填项")
        } else {
          let that = this;
          let site_ip = /^(\d|[1-9]\d|1\d{2}|2[0-5][0-5])\.(\d|[1-9]\d|1\d{2}|2[0-5][0-5])\.(\d|[1-9]\d|1\d{2}|2[0-5][0-5])\.(\d|[1-9]\d|1\d{2}|2[0-5][0-5])$/
          if (!site_ip.test(that.param.mac)) {
            that.hintInfo("warning", "IP地址输入错误，请输入4段0-255之间的数字，并以.隔开")
          } else {
            let urldata = that.Edite ? (that.url + "/v1/finance/cash_register/update/" + that.id) : (that.url + "/v1/finance/cash_register/add");
            ...
            that.$axios({
              ...
              .then(res => {
                if (res.data.message == "success") {
                  ...
                  if (that.Edite) {
                    that.hintInfo("success", "修改收银点成功")
                  } else {
                    that.hintInfo("success", "新增收银点成功")
                  }
                }
                else {
      ...
      handleCurrentChange(curPage) {
        ...
        //获取信息列表
        that.$axios({
          url: that.url + "/v1/common/module/info_list",
          method: "get",
          params: {
            page_num: curPage,
