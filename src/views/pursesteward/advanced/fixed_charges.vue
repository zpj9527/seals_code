    ...
    <div class="shopping">
      <div style="position: relative">
        <span
          ...
          @click="screen_all" title="所有固定支出">固定支出</span>
      ...
      <el-dialog
        ...
        <ul class="dialog_style">
          ...
          <li>
            <i style="color: red;font-size: 18px">*</i>
            <span>入账代码 ：</span>
            <el-select v-model="code_income_type_id" placeholder="请选择入账代码" style="width: 200px;" size="mini">
              <el-option
          ...
          <li>
            <i style="color: red;font-size: 18px">*</i>
            <span>消费名称 ：</span>
            <el-input v-model="param.name" placeholder="请输入消费名称" style="width: 200px;" size="mini"></el-input>
          </li>
          <li>
            <i style="color: red;font-size: 18px">*</i>
            <span>金额 ：</span>
            <el-input v-model="param.charge" placeholder="请输入金额" style="width: 200px;" size="mini"
  ...
  export default {
    ...
    methods: {
      ...
      enter_code() {
        let that = this
        that.$axios({
          url: that.url + "/v1/finance/code_fixed_charge/info_list",
          method: "get",
        })
          .then(res => {
            if (res.data.message == "success") {
      ...
      cancel(id, index, list) {
        ...
        that.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          list.splice(index, 1);
          that.$axios({
            url: that.url + "/v1/finance/code_fixed_charge/remove/" + id,
            method: "get",
          })
            .then(res => {
              console.log(res)
      ...
      //点击入账代码新增和修改后的保存
        ...
        if (that.param.code === "" || that.code_income_type_id === "" || that.param.name === "" || that.param.charge === "" || that.is_sys === ""
          ...
          let urldata = that.Edite ? (that.url + "/v1/finance/code_fixed_charge/update/" + that.id) : (that.url + "/v1/finance/code_fixed_charge/add");
          ...
          that.$axios({
            ...
            .then(res => {
              if (res.data.message == "success") {
                ...
                if (that.Edite) {
                  that.hintInfo("success", "修改固定支出成功")
                } else {
                  that.hintInfo("success", "新增固定支出成功")
                }
              }
              else {
