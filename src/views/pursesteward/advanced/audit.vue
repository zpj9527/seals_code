<template>
  <div class="audit">
    ...
    <div class="shopping">
      <span
        style="vertical-align: top;display: inline-block;line-height: 35px;padding:10px;font-weight: bold">查账原因</span>
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              ...
              <el-button size="mini"
                         @click="dialogVisible = true;amend(scope.row,scope.row.id,scope.$index, income_list) " v-if="scope.row.is_sys === 1" disabled>修改
              </el-button>
              <el-button size="mini"
    ...
    methods: {
      ...
      cancel(id, index, list) {
        ...
        that.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          list.splice(index, 1);
          that.$axios({
            url: that.url + "/v1/finance/check_account_reason/remove/" + id,
            method: "get",
          })
            .then(res => {
              console.log(res)
      ...
      //查账原因的新增和修改后的保存
        ...
        let that = this;
        if (!that.is_sys || !that.is_group || !that.name || !that.param.desc) {
          that.hintInfo("warning", "*为必填项")
        } else {
          let urldata = that.Edite ? (that.url + "/v1/finance/check_account_reason/update/" + that.id) : (that.url + "/v1/finance/check_account_reason/add");
          ...
          that.$axios({
            ...
            .then(res => {
              if (res.data.message == "success") {
                ...
                if (that.Edite) {
                  that.hintInfo("success", "修改查账原因成功")
                } else {
                  that.hintInfo("success", "新增查账原因成功")
                }
              }
              else {
