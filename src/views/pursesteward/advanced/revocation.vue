<template>
  <div class="revocation">
    ...
    <div class="shopping">
      <span
        style="vertical-align: top;display: inline-block;line-height: 35px;padding:10px;font-weight: bold">撤销原因</span>
      ...
      <el-dialog
        ...
        <ul class="dialog_style">
          <li>
            ...
            <el-select v-model="the_type" placeholder="请选择" style="width: 200px;" size="mini">
              <el-option
          ...
          <li>
            ...
            <el-input v-model="param.desc_cn" placeholder="请输入内容" style="width: 200px;" size="mini"></el-input>
          </li>
  ...
  export default {
    ...
    methods: {
      ...
      enter_code() {
        let that = this
        that.$axios({
          url: that.url + "/v1/finance/accounts_cancel_reason/info_list",
          method: "get",
        })
          .then(res => {
      ...
      cancel(id, index, list) {
        ...
        that.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          list.splice(index, 1);
          that.$axios({
            url: that.url + "/v1/finance/accounts_cancel_reason/remove/" + id,
            method: "post",
          })
            .then(res => {
              console.log(res)
      ...
      //撤销原因的新增和修改后的保存
        ...
        if (!that.the_type || !that.is_sys || !that.is_group || !that.name) {
          ...
          let urldata = that.Edite ? (that.url + "/v1/finance/accounts_cancel_reason/update/" + that.id) : (that.url + "/v1/finance/accounts_cancel_reason/add");
          ...
          that.$axios({
            ...
            .then(res => {
              if (res.data.message == "success") {
                ...
                if (that.Edite) {
                  that.hintInfo("success", "修改撤销原因成功")
                } else {
                  that.hintInfo("success", "新增撤销原因成功")
                }
              }
              else {
