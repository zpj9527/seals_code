    ...
    <div v-show=" listShow == false " style="width: 1660px;" class="MemberList">
      ...
      <div v-if="newAddBtn||canEdit" class="memberMes">
        ...
        <div class="link">
          ...
          <div class="inputCell">
            <p>房价码：</p>
            <el-select v-model="certainBaseData.rate_code" filterable clearable placeholder="请选择" style="width: 250px;" >
        ...
        <div class="resetBtn">
          ...
          <el-button type="primary" class="btn" disabled>消费密码</el-button> 
            <el-button type="primary" class="btn"  @click="navigateToDetail('/IntegralList')">积分</el-button> 
          <el-button type="primary" class="btn" disabled  @click="navigateToDetail('/StoredList')">储值</el-button>
        </div>
      </div>
      <div v-else class="memberMes">
        ...
        <div class="resetBtn">
          ...
          <el-button type="primary" class="btn">消费密码</el-button>
            <el-button type="primary" class="btn"  @click="navigateToDetail('/IntegralList')">积分</el-button> 
          <el-button type="primary" class="btn" @click="navigateToDetail('/StoredList')">储值</el-button>
        </div>
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    */
    navigateToDetail(router){
        this.$router.push({ path:router,query:{ memberData: this.certainBaseData } })
    },
    // 修改状态激活按钮封装函数
    ifStatus(status) {
    ...
    get_card_base_list() {
      ...
      let url =
          this.UrLHeader +
          "customer/member/get_card_base_list/?page=1&page_size=15",
        _this = this;
      // 发送请求:将数据返回到一个回到函数中
          ...
          if (res.data.message == "success") {
            if (res.data.data.results.length) {
              ...
              for (let i in _this.cardBaseList) {
                ...
                //     _this.cardBaseList[i].card_type = _this.CardTypeTree[m].descript;
                //     //会员等级转换
                //     for (let n in _this.CardTypeTree[m].children) {
                //       if ( (_this.cardBaseList[i].card_level = _this.CardTypeTree[m].children[n].code) ) {
                //         _this.cardBaseList[i].card_level = _this.CardTypeTree[m].children[n].descript;
                //       }
                //     }
                //   }
                // }
        ...
        // if (_this.id_code_reg.test(cardBase.id_no)) {
          if (_this.phone_reg.test(cardBase.phone)) {
            axios({
              ...
              .then(function(res) {
                if (res.data.message == "success") {
                  _this.get_part(res.data.data.id) ;
                  _this.get_card_base_list();
                  _this.$message({
                    message: "新增成功",
                    type: "success"
                  });
                  _this.newAddBtn = false;
                } else {
                  _this.$message.error("新增信息失败");
                }
              })
              .catch(error => {});
          } else {
    ...
    get_card_level_tree() {
      let that = this; 
      if (that.CardTypeTree.length) {
        for (let i in that.CardTypeTree) { 
          if (that.certainBaseData.card_type == that.CardTypeTree[i].id) {
            that.CardLevelTree = that.CardTypeTree[i].children; 
          }
        }
      } else {
        that.$message({
