      ...
      <el-tabs v-model="activeName2" type="card" @tab-click="handleClick">
          ...
          <el-dialog
            ...
            <ul class="dialog_style">
              ...
              <li>
                ...
                <el-select v-model="room_no" placeholder="请选择" size="mini" style="width: 217px;">
                  <el-option
                    v-for="item in room_number"
                    :key="item.value"
                    :label="item.room_no"
              ...
              <li>
                ...
                <el-select v-model="status" placeholder="请选择" size="mini" style="width: 217px;">
                  <el-option
              ...
              <li>
                <i style="color: red;font-size: 18px">*</i>
                <span>出发地 ：</span>
                <el-input v-model="param.origin" placeholder="请输入内容" style="width: 217px;" size="mini"></el-input>
              </li>
              <li>
                <i style="color: red;font-size: 18px">*</i>
                <span>目的地 ：</span>
                <el-input v-model="param.destination" placeholder="请输入内容" style="width: 217px;" size="mini"></el-input>
              </li>
              <li>
                <i style="color: red;font-size: 18px">*</i>
                <span>预定人数 ：</span>
                <el-input v-model="param.people_number" placeholder="请输入内容" style="width: 217px;"
                          size="mini"></el-input>
              </li>
              <li>
                <i style="color: red;font-size: 18px">*</i>
              ...
              <li>
                <i style="color: red;font-size: 18px">*</i>
                <span>手机号 ：</span>
                <el-input v-model="param.phone" placeholder="请输入内容" maxlength="11" size="mini"
                          oninput="value=value.replace(/[^\d]/g,'')" style="width: 217px;"></el-input>
              </li>
              <li>
                <i style="color: red;font-size: 18px">*</i>
                <span>车次类型：</span>
                <el-select v-model="car_type" placeholder="请选择" size="mini" style="width: 217px;">
                  <el-option
              ...
              <li>
                <i style="color: red;font-size: 18px">*</i>
                <span>酒店车辆名称 ：</span>
                <el-select v-model="hotel_bus_name" placeholder="请选择" size="mini" style="width: 217px;">
                  <el-option
          ...
          <el-dialog
            ...
            <ul class="dialog_style">
              <li>
                ...
                <el-select v-model="borrowers_man" placeholder="请选择" size="mini" style="width: 248px;">
                  <el-option
        ...
        <el-tab-pane label="已完成" name="routing">
          <div class="grabble">
            <div>
              <span>房间号 ：</span>
              <el-select v-model="achieve_room" placeholder="请选择" size="mini" style="width: 150px;">
                <el-option
                  clearable
                  v-for="item in room_number"
                  :key="item.room_no"
        ...
        <el-tab-pane label="已删除" name="delete">
          <div class="grabble">
            <div>
              <span>房间号 ：</span>
              <el-select v-model="delete_room" placeholder="请选择" size="mini" style="width: 150px;">
                <el-option
                  clearable
                  v-for="item in room_number"
                  :key="item.room_no"
  ...
  export default {
    ...
    methods: {
      ...
      receiver() {
        let that = this
        that.$axios({
          url: 'http://47.98.113.173:9530/v1/manage/duty_record/on_duty_employees',
          method: "get",
        })
          .then(res => {
            if (res.data.message == "success") {
      ...
      enter_code() {
        let that = this;
        that.$axios({
          url: that.url + "/v1/task/room_service/regular_bus/get_rs_regular_bus_list/",
          method: "get",
        })
          .then(res => {
            if (res.data.message == "success") {
      ...
      cancel(id, index, list) {
        ...
        that.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          list.splice(index, 1);
          that.$axios({
            url: that.url + "/v1/task/room_service/regular_bus/remove_rs_regular_bus/" + id + '/',
            method: "post",
          })
            .then(res => {
              console.log(res)
      ...
      //点击未接待新增和修改后的保存
        ...
        if (that.param.master_single_num === "" || that.room_no === "" || that.status === "" || that.param.origin === "" || that.param.destination === "" || that.param.people_number === ""
          ...
          if (!tel.test(that.param.phone)) {
            ...
            let urldata = that.Edite ? (that.url + "/v1/task/room_service/regular_bus/update_rs_regular_bus/" + that.id + '/') : (that.url + "/v1/task/room_service/regular_bus/add_rs_regular_bus/");
      ...
      determine() {
        ...
        if (that.borrowers_man === "") {
          ...
          that.$axios({
            url: that.url + "/v1/task/room_service/regular_bus/update_rs_regular_bus/" + that.id + '/',
            method: "post",
            data: {
              status: parseInt(that.status),//订单状态
      ...
      * */
      ...
      filtrate() {
        ...
        if (that.not_room === "" && that.reception_time === "") {
          ...
          that.$axios({
            url: that.url + "/v1/task/room_service/regular_bus/get_rs_regular_bus_list/?room_no=" + that.not_room + "&" + "plan_start_time=" + that.reception_time,
            method: "get",
          })
            .then(res => {
              if (res.data.message == "success") {
      ...
      reception_filtrate() {
        ...
        if (that.reception_room === "" && that.reception_data === "" && that.reception_man === "") {
          ...
          that.$axios({
            url: that.url + "/v1/task/room_service/regular_bus/get_rs_regular_bus_list/?room_no=" + that.reception_room + "&" + "plan_start_time=" + that.reception_data + "&" + "complete_user_name=" + that.reception_man,
            method: "get",
          })
            .then(res => {
              if (res.data.message == "success") {
      ...
      achieve_filtrate() {
        ...
        if (that.achieve_room === "" && that.achieve_time === "" && that.achieve_man === "") {
          ...
          that.$axios({
            url: that.url + "/v1/task/room_service/regular_bus/get_rs_regular_bus_list/?room_no=" + that.achieve_room + "&" + "plan_start_time=" + that.achieve_time + "&" + "complete_user_name=" + that.achieve_man,
            method: "get",
