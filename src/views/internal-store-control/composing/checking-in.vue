  ...
  <div class="checking_in">
    ...
    <div class="shopping">
      <div class="head" style="width: 100%;height: 40px;margin-top: 10px">
        <span style="vertical-align: top;display: inline-block;margin-left: 10px;margin-top: 4px;margin-right: 10px">班次设置列表</span>
      ...
      <el-dialog
        ...
        <ul class="dialog_style">
          <li>
            ...
            <span>中文描述 ：</span>
            <el-input v-model="param.desc_cn" placeholder="请输入中文描述" clearable style="width: 220px;"
                      size="mini"></el-input>
          </li>
          <li>
            <i style="color: red;font-size: 18px">*</i>
            <span>班次代码 ：</span>
            <el-input v-model="param.code" placeholder="请输入班次代码" clearable style="width: 220px;" size="mini"></el-input>
          </li>


          <!--<li>-->
          <!--<span style="margin-left: 10px">英文描述 ：</span>-->
          <!--<el-input v-model="param.desc_en" placeholder="请输入内容" style="width: 220px;" size="mini"></el-input>-->
          <!--</li>-->

          <!--<li>-->
          <!--<i style="color: red;font-size: 18px">*</i>-->
          <!--<span>每周工作天数 ：</span>-->
          <!--&lt;!&ndash;<el-input v-model="param.in_or_out" placeholder="请输入内容" style="width: 200px;"></el-input>&ndash;&gt;-->
          <!--<el-select v-model="days_per_week" placeholder="请选择" style="width: 220px;" size="mini">-->
          <!--<el-option-->
          ...
          <!--<i style="color: red;font-size: 18px">*</i>-->
          <!--<span>所属部门 ：</span>-->
          <!--<el-select v-model="dept_id" placeholder="请选择" style="width: 220px;" size="mini">-->
          <!--<el-option-->
          ...
          <li>
            <i style="color: red;font-size: 18px">*</i>
            <span>计薪单位 ：</span>
            <el-select v-model="salary_unit" placeholder="请选择计薪单位" style="width: 220px;" size="mini" clearable>
              <el-option
          ...
          <li>
            <i style="color: red;font-size: 18px">*</i>
            <span>最低单位薪资 ：</span>
            <el-input v-model="param.unit_salary" placeholder="请输入单位薪资" style="width: 220px;" size="mini"
          ...
          <li>
            ...
            <textarea
              ...
              v-model="param.rest_period_remark"></textarea>
          </li>
    ...
    methods: {
      ...
      flush() {
        ...
        that.days_per_week = '',//每周工作天数
          ...
          that.end_time = '',//终止时间
          that.time_continuous = '1',//工作时间是否连续
          ...
          .then(res => {
            ...
            if (res.data.message === "success") {
              ...
              for(let item of res.data.data.list){
                console.log(item.id, item.desc_cn);
              }
      ...
      cancel(id, index, list) {
        ...
        that.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          that.$axios({
            url: "" + that.url + "/v1/manage/duty_shift/delete/" + id,
            method: "get",
          })
            .then(res => {
              console.log(res)
      ...
      //点击入账代码新增和修改后的保存
        ...
        let that = this;
        if (!that.param.code || !that.start_time || !that.end_time || !that.time_continuous || !that.param.desc_cn
          || !that.salary_unit || !that.param.unit_salary
        ) {
          that.hintInfo("warning", "*为必填项")
        } else {
          let urldata = that.Edite ? (that.url + "/v1/manage/duty_shift/update/" + that.id) : ("" + that.url + "/v1/manage/duty_shift/add");
            ...
            .then(res => {
              if (res.data.message == "success") {
                ...
                if (that.Edite) {
                  ...
                  that.hintInfo("success", "新增班次成功")
                }
              }
              else {
                if (that.Edite) {
                  that.hintInfo("success", "修改班次失败")
                } else {
                  that.hintInfo("success", "新增班次失败")
                }
              }
            })
            .catch(error => {
