  ...
  <div class="reserve">
    ...
    <div class="label">
      <el-tabs v-model="activeName2" type="card" @tab-click="handleClick">
        ...
        <!--超预留设置-->
        <el-tab-pane label="超预留设置" name="second">
          <!--<div class=" search">-->
          <!--<el-input v-model="input" placeholder="请输入内容" style="width: 210px;" size="mini"></el-input>-->
          <!--<button>搜索</button>-->
          <!--</div>-->
        ...
        <!--取消预定原因-->
        <el-tab-pane label="取消预定原因" name="source">
          <div class=" search">
            <el-input v-model="input" placeholder="请输入内容" style="width: 210px;" size="mini"></el-input>
            <button>搜索</button>
          </div>
          <button style="width: 80px;height: 30px;margin-top: 16px;background: #4488E9FF;color: white;outline: none; border: none;
          border-radius: 4px;line-height: 30px ;margin-bottom: 24px" @click="src_add();dialogsrc=true;flush()">新增
          </button>
          <!--取消预定原因-->
          <el-table
            :data="src_Data"
          ...
          <el-dialog
            ...
            <span slot="footer" class="dialog-footer">
          <el-button @click="src_cancel()" type="danger" v-show="delete_id" size="mini">删除</el-button>
        ...
        <!--订房定金规则-->
        <el-tab-pane label="订房定金规则" name="ditch">订房定金规则暂时没有接口</el-tab-pane>
        <el-tab-pane label="订房取消规则" name="signing">
          <!--<div class=" search">-->
  ...
  export default {
    ...
    methods: {
      handleClick(tab, event) {
        ...
        if (tab.label === "预定类型") {
          that.rmchg_reason();
        } else if (tab.label === "超预留设置") {
          that.gain_market();
        } else if (tab.label === "取消预定原因") {
          that.gain_src();
        } else if (tab.label === "订房定金规则") {
          that.gain_channel();
        } else if (tab.label === "订房取消规则") {
      ...
      rmchg_reason() {
        ...
        //获取信息列表
        that.$axios({
          url: that.url + "/v1/system/settings/get_code_base_list/?parent_code=rmchg_reason",
          method: "get",
        })
          .then(res => {
      ...
      gain_src() {
        ...
        //获取信息列表
        that.$axios({
          url: that.url + "/v1/system/settings/get_code_base_list/?page_size=100&parent_code=resrv_cancel&ordering=-modify_datetime",
          method: "get",
        })
          .then(res => {
            if (res.data.message == "success") {
      ...
      gain_channel() {
        ...
        //获取信息列表
        that.$axios({
          url: that.url + "/v1/system/settings/get_code_base_list/?page_size=100&parent_code=channel&ordering=-modify_datetime",
          method: "get",
        })
          .then(res => {
            if (res.data.message == "success") {
      ...
      gain_exchangeChannel() {
        ...
        //获取信息列表
        that.$axios({
          url: that.url + "/v1/system/settings/get_code_base_list/?page_size=100&parent_code=exchangeChannel&ordering=-modify_datetime",
          method: "get",
        })
          .then(res => {
            if (res.data.message == "success") {
