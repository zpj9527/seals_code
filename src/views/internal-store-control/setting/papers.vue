<template>
  <div class="papers">
    <div class="title">
      ...
      <span style="color:#777777FF;font-size: 18px">基础信息设置</span>
    </div>
    <div class="label">
      <el-tabs v-model="activeName2" type="card" @tab-click="handleClick">
        <el-tab-pane label="一般证件类型" name="first">
          <div class=" search">
            <el-input v-model="input" placeholder="请输入内容" style="width: 217px;" size="mini"></el-input>
        ...
        <el-tab-pane label="签证代码设置" name="second">
          ...
          <button style="width: 80px;height: 30px;margin-top: 16px;background: #4488E9FF;color: white;outline: none; border: none;
          border-radius: 4px;line-height: 30px;margin-bottom: 24px" @click="increase();dialog=true;flush()">新增
          </button>
          <!--证件类型-->
          <el-table
            :data="vsia_list"
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <el-button size="small"
                           @click="dialog_submit(scope.row,scope.row.id,scope.$index, vsia_list);dialog=true">修改
                </el-button>
              </template>
          ...
          <el-dialog
            ...
            <span slot="footer" class="dialog-footer">
              <el-button @click="dialog_cancel()" type="danger" v-show="delete_id" size="mini">删除</el-button>
              <el-button @click="dialog = false" size="mini">取 消</el-button>
              <el-button type="primary" @click="dialog_ensure()" size="mini">确 定</el-button>
        ...
        <el-tab-pane label="签证机关设置" name="third">
          ...
          <button style="width: 80px;height: 30px;margin-top: 16px;background: #4488E9FF;color: white;outline: none; border: none;
          border-radius: 4px;line-height: 30px;margin-bottom: 24px" @click="organ_add();dialog_organ=true;flush()">新增
          </button>
          <!--证件类型-->
          <el-table
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                <el-button size="small"
                           @click="organ_submit(scope.row,scope.row.id,scope.$index, visa_office_list);dialog_organ=true">
                  修改
                </el-button>
              </template>
          ...
          <el-dialog
            ...
            <span slot="footer" class="dialog-footer">
              <el-button @click="organ_delete()" type="danger" v-show="delete_id" size="mini">删除</el-button>
              <el-button @click="dialog_organ = false" size="mini">取消</el-button>
              <el-button type="primary" @click="organ_ensure()" size="mini">确定</el-button>
    ...
    methods: {
      ...
      handleClick(tab, event) {
        ...
        if (tab.label === '一般证件类型') {
          //获取证件列表
          that.papers_id();
          that.api_url = '/v1/system/settings/get_code_base_list/?parent_code=idcode&ordering=-modify_datetime'
        } else if (tab.label === '签证代码设置') {
          //获取签证设置的列表
          that.visa_id();
          that.api_url = '/v1/system/settings/get_code_base_list/?page_size=100&parent_code=visaid&ordering=-modify_datetime'
        } else if (tab.label === '签证机关设置') {
          //签证机关设置
          that.visa_office();
          that.api_url = '/v1/system/settings/get_code_base_list/?parent_code=visaunit&ordering=-modify_datetime'
        }
      },
      //点击分页上的页数
        ...
        that.$axios({
          ...
          .then(res => {
            if (res.data.message == "success") {
              ...
              if (that.api_url === '/v1/system/settings/get_code_base_list/?parent_code=idcode&ordering=-modify_datetime') {
                that.tableData = res.data.data.results;
              } else if (that.api_url === '/v1/system/settings/get_code_base_list/?page_size=100&parent_code=visaid&ordering=-modify_datetime') {
                that.vsia_list = res.data.data.results;
              } else if (that.api_url === '/v1/system/settings/get_code_base_list/?parent_code=visaunit&ordering=-modify_datetime') {
      ...
      papers_id() {
        ...
        //获取信息列表
        that.$axios({
          url: that.url + "/v1/system/settings/get_code_base_list/?parent_code=idcode&ordering=-modify_datetime",
          method: "get",
        })
          .then(res => {
            if (res.data.message == "success") {
      ...
      visa_id() {
        ...
        //获取信息列表
        that.$axios({
          url: that.url + "/v1/system/settings/get_code_base_list/?page_size=100&parent_code=visaid&ordering=-modify_datetime",
          "method": "get",
        })
          .then(res => {
            if (res.data.message == "success") {
      ...
      visa_office() {
        ...
        //获取信息列表
        that.$axios({
          url: that.url + "/v1/system/settings/get_code_base_list/?parent_code=visaunit&ordering=-modify_datetime",
          method: "get",
          params: {
            page_size: 10
          }
          ...
          that.$axios({
            ...
            .then(res => {
              if (res.data.message === "success") {
                ...
                if (that.Edite) {
                  that.hintInfo("success", "修改证件类型成功")
                } else {
                  that.hintInfo("success", "新增证件类型成功")
                }
              }
              else {
          ...
          that.$axios({
            ...
            .then(res => {
              if (res.data.message === "success") {
                ...
                that.dialog = false;
                if (that.Edite) {
                  that.hintInfo("success", "修改签证代码设置成功")
                } else {
                  that.hintInfo("success", "新增签证代码设置成功")
                }
              }
              else {
                that.error(res.data.message)
          ...
          that.$axios({
            ...
            .then(res => {
              if (res.data.message === "success") {
                ...
                if (that.Edite) {
                  that.hintInfo("success", "修改签证机关成功")
                } else {
                  that.hintInfo("success", "新增签证机关成功")
                }
              }
              else {
                that.error(res.data.message)
