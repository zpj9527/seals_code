<template>
  <div class="loginView">
    ...
    <el-form ref="AccountForm" :model="account"  label-position="left" label-width="0px" class="demo-ruleForm login-container">
      ...
      <h3 class="title" style="float: right" @click="more" v-show="multiple"> 多店登陆</h3>
       <div class="alone " v-if="alone_or_plural"  >
         <!-- 默认登录门店 -->
             ...
             <el-row :gutter="20">
               <el-col :span="24">
                 <div style="height: 238px;display: flex; align-items: center; justify-content: center;background: #FFFFFF;border-radius: 4px">
                   <img src="../assets/images/initial/clearlove.png" style="width: 190px;height: auto;" /><br/>
                 </div>
               </el-col>
             </el-row>
           </div>
        ...
        <div class="plural" v-else  >
          <div class="login_sty"  >
            <!--账号密码登录-->
            <!-- 用户姓名 -->
            <el-form-item prop="username" >
              <el-input type="text" v-model="more_username" auto-complete="off" placeholder="请输入用户名/手机号"></el-input>
            </el-form-item>
            <!-- 登录密码 -->
            <el-form-item prop="pwd" >
              <el-input type="password" v-model="more_pwd" :autofocus="pwdFocus" auto-complete="off" placeholder="请输入账号密码"></el-input>
            </el-form-item>
            <!-- 登录btn -->
            <el-form-item style="width:100%;" >
              <el-button type="primary" style="width:100%;"  v-show="login_btn" @click.native.prevent="handleLogin_more">登录
              </el-button>
            </el-form-item>
          </div>
        </div>
  ...
  export default {
    ...
    data() {
      return {
        ...
        multiple:true,//显示多店登陆的文字
        desc:null,//路由跳转携带的参数
        scheduleValue: '',
        alone_or_plural:true,
        more_username:'',
        more_pwd:'',//多店登陆密码
      };
    },
    ...
    created() {
      ...
      localStorage.getItem('more_username') ? this.more_username = localStorage.getItem('more_username') : '';
      localStorage.getItem('more_password') ? this.more_pwd = localStorage.getItem('more_password') : '';
      /**
       * 将登陆的事件绑定到回车键上
       * @param e
       * @returns {boolean}
    ...
    methods: {
        ...
        if(that.check_login === 'login_flag'){
          ...
          if(username.trim().length > 0 && pwd.trim().length > 0 ){
            this.$axios({
              ...
              console.info(res);
              if(res.data.message === "success") {
                console.log(res.data.root_level);
                ...
                if(that.desc==='控台'){
                  if(res.data.root_level!==1){
                    that.hintInfo('warning',"请选择系统管理员登陆");
                  }else {
                    that.$router.push({
                      path: '/index'
                    });
                    sessionStorage.setItem('desc',that.desc);//index
                  }
                }else {
                  that.$router.push({
                    path: '/index'
            ...
            .then(res => {
              if(res.data.message==="success"){
                ...
                for(let item of res.data.data){
                  if(item.desc_cn==="休息"&& item.id===null){
                    arr=[];
                    that.hintInfo('warning','您今天休息，祝您休息愉快！');
                  }else {
                    arr.push(item);
                  }
                }
      ...
      handleLogin_more(){
        let that =this;
        let url =  "http://organ.crowncrystalhotel.com/v1/organization/ht/employee/login/";
        let username = this.more_username;
        let pwd = this.more_pwd;
        if(username.trim().length > 0 && pwd.trim().length > 0 ){
          this.$axios({
            method: 'post',
            url: url,
            ...
            console.info(res);
            if(res.data.message === "success") {
              localStorage.setItem('more_username',username);//记住账号密码
              localStorage.setItem('more_password',pwd);//记住账号
              console.log(res.data.data.can_use_hotel);
              sessionStorage.setItem('username',username);
              sessionStorage.setItem('password',that.getmd5(pwd));
