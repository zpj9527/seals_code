              ...
              <table style="width: 100%;">
                ...
                <tbody>
                    <tr v-for="(item,index) in promotionList" :key="index" @click="get_card_promotion_info(item.id) ">
                        ...
                        <td>{{ trigger_wayDic[item.trigger_way] }}</td>
                        <td>{{ target_wayDic[item.target_way] }}</td>
              ...
              <!-- 单条内层 -->
                <div class="innerbox">
                    <div class="inner pull-left">
                        ...
                        <el-input v-model="CertainPromotion.code" placeholder="请输入内容" maxlength="10" style="width: 70%;"></el-input>
                    </div>
                    <div class="inner pull-right">
                        <p>停用：</p>
                        <el-radio v-model="CertainPromotion.is_halt" label='1'>是</el-radio>
                        <el-radio v-model="CertainPromotion.is_halt" label='0'>否</el-radio>
                    </div>
                </div> 
                <!-- 单条内层 -->
                <div class="innerbox">
                ...
                <div class="innerbox">
                    <div class="inner pull-left">
                        ...
                        <el-date-picker v-model="CertainPromotion.date_begin" type="date" placeholder="选择日期" style="width: 70%;"> 
                        </el-date-picker>
                    </div>
                    <div class="inner pull-right">
                ...
                <!-- 单条内层 --> 
                <div v-if=" CertainPromotion.trigger_way == 'card_charge' " class="innerbox">
                    <div class="inner pull-left">
                        <p>最小充值金额：</p>
                        <el-input type="number" v-model=" charge_form.min_pay " placeholder="请输入内容" style="width: 70%;"></el-input>
                    </div>
                    <div class="inner pull-right">
                        <p>最大充值金额：</p>
                        <el-input type="number" v-model=" charge_form.max_pay " placeholder="请输入内容" style="width: 70%;"></el-input>
                    </div>
                </div>
                <!-- 单条内层 --> 
                <div v-if=" CertainPromotion.trigger_way == 'card_charge' "  class="innerbox">
                    <div class="inner pull-left">
                        <p>是否首次充值：</p>
                        <el-checkbox v-model=" charge_form.is_first "></el-checkbox>
                    </div> 
                </div>
                <!-- 删除保存 -->
...
export default {
    ...
    data(){
        return{
            ...
            // 请求地址头
            UrLHeader:'http://member.crowncrystalhotel.com/v1/',//http://192.168.4.234:9102/v1/
            // 下拉框请求头
            SelectUrLHeader:"http://code.crowncrystalhotel.com/",//'http://47.98.113.173:9102/',
            // 分页
            currentPage: 1,
            pagesize: 10,
            total: 0,
            ...
            trigger_wayDic:{
              ...
              'login_in': '协议单位登录时',
              'upgrade': '协议单位升级时',
              'evaluation': '协议单位网站评价时',
              'sing_in': '协议单位网站签到时',
              'card_charge': '储值卡充值时',
              'room_check_out': '客房结账时',
              'register_card': '注册协议单位成功时'
            },
            // 奖励方式
            target_wayArr:[
              {
            ...
            /* 充值最大值和最小值 */
            charge_form:{ "is_first": true, "min_pay": 0, "max_pay": 0 }
        }
    },
    // 加载页面
    created(){
    ...
    methods:{
      ...
      get_card_promotion_info(id){
          ...
          axios.get(url ).then(function (res) {
            ...
            for(let m in that.CardTypeTree){
              if(that.CertainPromotion.card_type == that.CardTypeTree[m].id){
                  that.CertainPromotion.card_type = that.CardTypeTree[m].descript;
                      //协议单位等级转换
                  for(let n in that.CardTypeTree[m].children){
                    if(that.CertainPromotion.card_level == that.CardTypeTree[m].children[n].id){
                      that.CertainPromotion.card_level = that.CardTypeTree[m].children[n].descript;
                    }
                  }
              }
            }    
            if(that.CertainPromotion.cond_and_para){
              that.CertainPromotion.cond_and_para = JSON.parse(that.CertainPromotion.cond_and_para);
              if(that.CertainPromotion.cond_and_para.is_first){
                that.charge_form.is_first = true
              }else{
                that.charge_form.is_first = false
              }

              that.charge_form = { 
                "is_first": that.charge_form.is_first, 
                "min_pay": that.CertainPromotion.cond_and_para.min_pay, 
                "max_pay": that.CertainPromotion.cond_and_para.max_pay 
              }
            }
             
            // console.log('是否停用',that.CertainPromotion.is_halt);
            if(that.CertainPromotion.is_halt){
              that.CertainPromotion.is_halt = '1'
            }else{
              that.CertainPromotion.is_halt = '0'
            }
      ...
      add_card_base(){
        ...
        let is_first ;
        if(_this.charge_form.is_first){
          is_first = 1
        }else{
           is_first = 0
        }
        
        if(_this.CertainPromotion.trigger_way == 'card_charge'){
          _this.CertainPromotion.cond_and_para = _this.charge_form;
          _this.CertainPromotion.cond_and_para.is_first = is_first;
          _this.CertainPromotion.cond_and_para.min_pay = parseFloat(_this.CertainPromotion.cond_and_para.min_pay);
          _this.CertainPromotion.cond_and_para.max_pay = parseFloat(_this.CertainPromotion.cond_and_para.max_pay);
          _this.CertainPromotion.cond_and_para = JSON.stringify(_this.CertainPromotion.cond_and_para) ;
        }
      ...
      update_card_promotion_info(id){
        ...
        console.log(_this.CertainPromotion);
        if( _this.CertainPromotion.code && _this.CertainPromotion.descript && _this.CertainPromotion.descript_en && _this.CertainPromotion.date_begin && _this.CertainPromotion.date_end && _this.CertainPromotion.card_type && _this.CertainPromotion.card_level){ 
          ...
          axios({
            ...
            for(let i in that.promotionList){
              for(let j in that.target_memberArr){
                ...
                // 协议单位计划转换
                for(let m in that.CardTypeTree){
                  if(that.promotionList[i].card_type == that.CardTypeTree[m].id){
                      that.promotionList[i].card_type = that.CardTypeTree[m].descript;
                      //协议单位等级转换
                      for(let n in that.CardTypeTree[m].children){
                        if(that.promotionList[i].card_level == that.CardTypeTree[m].children[n].id){
        ...
        let _this=this;
          ...
          if(_this.CardTypeTree.length){
              ...
              for(let i in _this.CardTypeTree){
                ...
                if(_this.CertainPromotion.card_type == _this.CardTypeTree[i].id){
