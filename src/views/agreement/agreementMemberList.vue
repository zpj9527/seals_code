    ...
    <div v-show=" listShow == false " style="width: 1660px;" class="MemberList">
      ...
      <div class="memberMes">
        ...
        <div class="link">
          ...
          <div class="inputCell">
            ...
            <el-select v-model="certainBaseData.company_base.country" filterable placeholder="请选择" style="width: 250px;"  size="mini" @focus="get_country_tree()">
              <el-option
        ...
        <div class="link">
          ...
          <div class="inputCell" >
            ...
            <el-select
              ...
              clearable
              placeholder="请选择"
              style="width: 250px;"  size="mini" @focus="get_class2Arr_tree()"
              >
          ...
          <div class="inputCell" >
            ...
            <el-select
              ...
              clearable
              placeholder="请选择"
              style="width: 250px;"  size="mini" @focus="get_class3Arr_tree()"
              >
          ...
          <div class="inputCell" >
            ...
            <el-select
              ...
              clearable
              placeholder="请选择"
              style="width: 250px;"  size="mini" @focus="get_class4Arr_tree()"
              > 
              <el-option
          ...
          <div class="inputCell" >
            ...
            <el-select
              ...
              clearable
              placeholder="请选择"
              style="width: 250px;"  size="mini" @focus="get_guest_type_tree()"
              >
          ...
          <div class="inputCell" >
            ...
            <el-select
              ...
              clearable
              placeholder="请选择"
              style="width: 250px;"  size="mini" @focus="get_guest_grade_tree()"
              >
      ...
      <div v-if="newAddBtn||canEdit" class="memberMes">
        ...
        <div class="link">
          ...
          <div class="inputCell">
            <p>销售员：</p>
            <el-input size="mini" v-model="certainBaseData.company_base.salesman" placeholder="请输入内容" style="width: 250px;"></el-input>
            <!-- <el-select
             size="mini"
        ...
        <!-- 取消保存 -->
        <div class="resetBtn">
          <el-button type="primary" class="btn" disabled>注册打印</el-button>
          <el-button type="primary" class="btn" disabled>制卡</el-button>
          ...
          <router-link to="/IntegralList" style="display: inline-block;">
            <el-button type="primary" disabled class="btn">积分</el-button>
          </router-link>
      ...
      <div v-else class="memberMes">
        ...
        <div class="resetBtn">
          ...
          <router-link to="/IntegralList" style="display: inline-block;">
            <el-button type="primary" class="btn" @click="navigateToDetail('/agreementIntegralList')">积分</el-button>
          </router-link>
          <el-button type="primary" class="btn" @click="navigateToDetail('/agreementStoredList')">储值</el-button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      // 下拉框请求头
      SelectUrLHeader: "http://code.crowncrystalhotel.com/",
      // 请求地址头
      UrLHeader: "http://member.crowncrystalhotel.com/v1/", //http://member.crowncrystalhotel.com/47.98.113.173:8098
      // 分页
      currentPage: 1,
      pagesize: 15,
      total: 0,
  ...
  methods: {
    /*
        跳转路由携带参数
    */
    navigateToDetail(router){
        this.$router.push({ path:router,query:{ memberData: this.certainBaseData.company_base } })
    },
    // 修改状态激活按钮封装函数
    ifStatus(status) {
    ...
    // 获取协议单位所有列表get_card_base_list
    get_card_base_list() {
      //设置请求路径
      let url = this.UrLHeader + "customer/company/get_company_base_list/?page=1&page_size=15", _this = this;
      // 发送请求:将数据返回到一个回到函数中
    ...
    get_certain_card_base_list(id) {
      let url = this.UrLHeader + "customer/company/get_company_base_info/" + id + "/", _this = this;
      _this.showFunc(); 
      axios .get(url) .then(function(res) {
          if (res.data.message == "success") {
            ...
            // 会员计划转换
            for (let m in _this.CardTypeTree) {
              if ( _this.certainBaseData.company_base.card_type == _this.CardTypeTree[m].id ) {
                _this.certainBaseData.company_base.card_type = _this.CardTypeTree[m].descript;

                //会员等级转换
                for (let n in _this.CardTypeTree[m].children) {
                  if ( (_this.certainBaseData.company_base.card_level == _this.CardTypeTree[m].children[n].id) ) {
                    _this.certainBaseData.company_base.card_level =  _this.CardTypeTree[m].children[n].descript;
                  }
                }
              }
            }
            // 克隆数据
            _this.cloneData = res.data.data;
            localStorage.setItem('cloneData',JSON.stringify(_this.certainBaseData) );
          } else {
            _this.$message.error("请求服务器失败，稍后重试");
          }
        })
    ...
    get_info(id){
      ...
      axios .get(url) .then(function(res) {
          if (res.data.message == "success") {
            ...
            // 克隆数据
            _this.cloneData = res.data.data;
            localStorage.setItem('cloneData',JSON.stringify(res.data.data));
    ...
    // 增加请求后台接口
    add_card_base() {
      let addCaseUrl = this.UrLHeader + "customer/company/add_company_base/", _this = this;
        ...
        // if (_this.phone_reg.test(company_base.phone)) {
              ...
              .then(function(res) {
                if (res.data.message == "success") {
                  ...
                  _this.get_info(res.data.data.id);
                  _this.$message({
                    message: "添加成功",
                    type: "success"
                  });
    ...
    update_card_base(id) {
      ...
      let cloneData = JSON.parse(localStorage.getItem('cloneData'));

        for(let i in _this.certainBaseData){
          for(let j in _this.certainBaseData[i]){
            if( _this.certainBaseData[i][j] == cloneData[i][j] ){ 
              delete _this.certainBaseData[i][j];
            }  
          }
        }  
        console.log('原有数据',_this.certainBaseData);  
          // if (_this.phone_reg.test(company_base.phone)) {
              axios({
                url: updateCaseUrl,
                method: "post",
                data: _this.certainBaseData
              }).then(function(res) {
                  if (res.data.message == "success") {
                    _this.get_card_base_list();
                    _this.get_info(cloneData.company_base.id)
                    _this.$message({
                      message: "保存成功",
                      type: "success"
                    });
      ...
      if (_this.CardTypeTree.length) {
        for (let i in _this.CardTypeTree) {
          if (_this.certainBaseData.company_base.card_type == _this.CardTypeTree[i].id) {
            ...
            if(_this.CardTypeTree[i].children.length){
              _this.CardLevelTree = _this.CardTypeTree[i].children;
            }else{
              _this.$message({
                message: "请先创建一个协议单位等级",
                type: "warning"
              });
            }
          }
    ...
    get_fj_codeArr_tree() {
      //设置请求路径
      let url =  "http://price.crowncrystalhotel.com/v1/rate_code/get_rate_code_list/",
        _this = this;
    ...
    handleCurrentPageChange(currentPage) {
      let that = this;
      let url = that.UrLHeader + "customer/company/get_company_base_list/";
  ...
  created() {
    ...
    // this.get_guest_type_tree();
    // this.get_guest_grade_tree();
    // this.get_class1Arr_tree();
    // this.get_class2Arr_tree();
    // this.get_class3Arr_tree();
    // this.get_class4Arr_tree();
    // this.get_city_tree();
    //           this.get_province_tree();
    //           this.get_country_tree();
  }
};
</script>
